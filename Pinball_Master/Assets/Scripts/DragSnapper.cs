...
public class DragSnapper : UIBehaviour, IEndDragHandler, IBeginDragHandler
{
    ...
    public void OnBeginDrag(PointerEventData eventData)
    {
        ...
        StopCoroutine(SnapRect(direction == SnapDirection.Horizontal
            ? scrollRect.horizontalNormalizedPosition
            : scrollRect.verticalNormalizedPosition)); // if we are snapping, stop for the next input
    }

    public void OnEndDrag(PointerEventData eventData)
    {
        Debug.Log(direction == SnapDirection.Horizontal
            ? scrollRect.horizontalNormalizedPosition
            : scrollRect.verticalNormalizedPosition);
        StartCoroutine(SnapRect(direction == SnapDirection.Horizontal
            ? scrollRect.horizontalNormalizedPosition
            : scrollRect.verticalNormalizedPosition)); // simply start our coroutine ( better than using update )
    }

    public void NextSnap()
    {
        float nextStartNormal = direction == SnapDirection.Horizontal
            ? scrollRect.horizontalNormalizedPosition
            : scrollRect.verticalNormalizedPosition;
    ...
    private IEnumerator SnapRect(float startNormal)
    {
        ...
        float delta = 1f / (itemCount - 1); // percentage each item takes
        int target = Mathf.RoundToInt(startNormal /
                                      delta); // this finds us the closest target based on our starting point
        ...
        while (timer < 1f) // loop until we are done
        {
            ...
            float value = Mathf.Lerp(startNormal, endNormal,
